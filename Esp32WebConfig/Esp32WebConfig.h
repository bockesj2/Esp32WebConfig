...
the Esp32 configures itself as an access point (AP) at 192.168.4.1 (note that a static ip is actually assigned) with an SSID of
...
The program adds items to the "Esp32WebConfig" object and these items are iterated through to create the configuration web page, save 
the items to NVM and produce a JSON string which can be used in other ways (such as through the serial port) in order to edit the
configuration for the processor.

Note that as the parameters are configured, a pointer to the underlying object is obtained which allows access to the
configuration values which are stored in non-volatile memory (NVM) or have been submitted using the web page. 
...
public:
    ...
    static HtmlItem_FieldSet* addFieldSet(String label);
    static void fieldSetEnd();
    ...
    static String createHtmlPage(String Title, String pageId);
